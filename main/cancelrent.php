...
class CancelRent {
    ...
    public function cancelRental(int $rent_id):string{ //function to cancel rent //: string is to handle the return type 
        ...
        $stmt = $this->conn->prepare("UPDATE rentedcar SET status = 'completed' WHERE rent_id = :rent_id"); //if the user cancel the rent the status will be updfate to completed and display in history
